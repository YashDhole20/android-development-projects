...
public class YoutubeViews extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_youtube_views);

        button = findViewById(R.id.btnsumbit);
        listView = findViewById(R.id.questionlistView);

        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                Log.d("SelectedOptions", selectedOptionsMap);
                ...
                String rs=Network.connect("http://"+Network.IP+"/Level1_ans.php",m);
                if (rs != "0") {
                    ...
                    Toast.makeText(YoutubeViews.this, "Failed to submit answers. Please try again later.", Toast.LENGTH_SHORT).show();
                }
            }
        });

        String response = Network.connect("http://" + Network.IP + "/traffic.php", new HashMap<>());
        List<String> buses = Arrays.asList(response.trim().split("#"));

        List<String[]> busDataList = new ArrayList<>();
        for (String questionsString : buses) {
            String[] busData = questionsString.split("<br>");
                    ...
                    for (int i = 0; i < radioButtons.length; i++) {
                        ...
                        radioButtons[i].setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                selectedOptionsMap.put(questionText, selectedOption); // Map question text to selected option
                            }
                        });
                    }
